...
const RiskProfile = () => {
    ...
    const InquiryForm = () => {
        ...
        return (
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 rounded py-3 px-6">
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormControl>
                                    <textarea placeholder="Message" {...field} className="w-full border-2 border-gray-500 p-1 bg-stone-900 rounded" aria-label="Message" />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
...
Here are the answers you provided:
    ...
    return (
        <div className="max-w-3xl mx-auto my-40 p-6 bg-gray-50 ring-1 ring-yellow-400 shadow-lg rounded-lg">
            {!isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-90">
                    <div className="p-3 rounded-lg shadow-lg w-[30rem] bg-gray-100 ring-1 ring-gray-800 mt-10">
                        <InquiryForm onClose={() => {
                            setIsModalOpen(false);
            ...
            {isQuizCompleted ? (
                <div className="flex flex-col items-center text-center">
                    <h2 className="text-3xl font-bold mb-4">Your Total Score: {score}</h2>
                    ...
                    <div className="bg-gray-100 p-4 rounded-lg shadow-inner">
                        ...
                        <ul className="mt-2 text-left">
                            <li className="mb-3"><span className="text-bold text-lg text-gray-900">Conservative: </span> Conservative investors are investors who want stability and are more concerned with protecting their current investments than increasing the real value of their investments.</li>
                            <li className="mb-3"><span className="text-bold text-lg text-gray-900">Moderately Conservative: </span>M Moderately conservative investors are investors who want to protect their capital, and achieve some real increase in the value of their investments.</li>
                            <li className="mb-3"><span className="text-bold text-lg text-gray-900">Moderate: </span>Moderate investors are long-term investors who want reasonable but relatively stable growth. Some fluctuations are tolerable, but investors want less risk than that attribute to a fully equity based investment.</li>
                            <li className="mb-3"><span className="text-bold text-lg text-gray-900">Moderately Aggressive: </span>Moderately Aggressive investors are long-term investors who want good real growth in their capital. A fair amount of risk is acceptable.</li>
                            <li className="mb-3"><span className="text-bold text-lg text-gray-900">Aggressive:</span> Aggressive investors are long-term investors who want high capital growth. Substantial year-to-year fluctuations in value are acceptable in exchange for a potentially high long-term return.</li>
                        </ul>
                    </div>
                    <button
                        onClick={() => {
                        ...
                        className="mt-6 bg-black text-white px-4 py-2 rounded-lg hover:bg-yellow-600"
                    >
                        Back Home
                    </button>
                </div>
                        ...
                        {questions[currentQuestionIndex]?.answers?.map((answer, index) => (
                            <div key={index} className="flex items-center mb-2">
                                <input
                                    ...
                                    className="mr-2"
                                />
                                <label htmlFor={`answer - ${index} `} className="text-lg">{answer.text}</label>
                            </div>
                        ))}
                    </div>
                    <Button
                        onClick={() => handleNextClick(questions[currentQuestionIndex]?.question)}
                        className="secondarybgcolor text-white px-4 py-2 rounded-lg hover:bg-yellow-600"
                    >
                        Next
                    </Button>
                </div>
