                                        ...
                                        <ul className="">
                                            {sub.children.map((child, childIndex) => (
                                                <li key={childIndex} className="mb-1">
                                                    <Link href={child.link} target="blank" className="hover:text-[var(--rv-primary)]"><p>{child.name}</p></Link>
                                                </li>
                                            ))}
                                        </ul>
                                    )}
                            ...
                            <li className="mb-5">
                                <div className="text-gray-50 font-medium">
                                    <div className="mb-4 flex items-center">
                                        ...
                                        <Link href={`tel:${sitedata?.mobile?.slice(0, 10)}`} className="hover:underline ml-3">{sitedata?.mobile?.slice(0, 10)}</Link>,
                                        <Link href={`tel:${sitedata?.mobile?.slice(12)}`} className="hover:underline ml-3">{sitedata?.mobile?.slice(12)}</Link>
                                    </div>
                                    <div className="mb-4 flex items-center">
                                        <p className="text-2xl font-semibold uppercase w-9 h-9 bg-white rounded-full flex items-center justify-center">
                                            <IoMail className="text-[var(--rv-primary)]" />
                                        </p>
                                        <Link href={`mailto:${sitedata?.email}`} className="hover:underline ml-3">
                                            {sitedata?.email}
                                        </Link>
                                    </div>
                                    <div className="mb-4 flex  items-center">
                                        ...
                                        <Link href={`mailto:${sitedata?.address}`} className="hover:underline w-48 ml-3">
                                            {sitedata?.address}
                                        </Link>
                                    </div>
                                </div>
              ...
              {iconMap[link.title] || <FaXTwitter className="text-[var(--rv-primary)]" />}
                ...
                <div className="text-gray-50 py-3 flex gap-x-3 justify-center">
                    <div className="flex gap-x-8 justify-center">
                        <div className="flex gap-x-3 justify-center">
                            <Image src={"/images/amfi-logo.jpg"} width={100} height={100} alt="image" className="rounded" />
                            <div>
                                <p>ARN - {arn[0]?.arn}</p>
                ...
                <div className="text-gray-50 w-full mx-auto max-w-screen-xl p-4 md:flex md:justify-between border-t border-gray-300">
                    <p className="sm:text-center">© 2025 <Link href="/" className="hover:underline">{sitedata?.websiteName}</Link>. All Rights Reserved.</p>
                    <ul className="flex flex-wrap items-center mt-3  sm:mt-0">
                        <li>
                            <Link target="_blank" href="https://www.ethicuswealth.com/" className="hover:underline me-4 md:me-6"><p>Designed & Developed by {sitedata?.websiteName}</p></Link>
                        </li>
                    </ul>
                </div>
            </div>
